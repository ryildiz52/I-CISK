...
for (filepath in geopackage_files) {
  ...
  # Group by station, year, and month, and summarize the values
  monthly_data <- data %>%
    group_by(station_id, station_name, year, month) %>%
    summarize(
  ...
  monthly_data_filtered <- monthly_data %>%
    group_by(station_id, station_name, year, month) %>%
    filter(n() > 0)  # Keep rows with at least one entry for the month
  # Create the filename for the aggregated data
  aggregated_filename <- paste0(year, "_agg_months.gpkg")
  aggregated_filepath <- file.path(geopackage_path, aggregated_filename)
  # Save the aggregated monthly data as a new GeoPackage file
  st_write(monthly_data_filtered, aggregated_filepath)
}  
